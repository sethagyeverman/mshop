...
func (m *OrderStatus) validate(all bool) error {
	...
	if m.GetId() <= 0 {
		err := OrderStatusValidationError{
			field:  "Id",
			reason: "value must be greater than 0",
		}
		if !all {
	...
	if l := utf8.RuneCountInString(m.GetOrderSn()); l < 1 || l > 30 {
		err := OrderStatusValidationError{
			field:  "OrderSn",
	...
	if _, ok := _OrderStatus_Status_InLookup[m.GetStatus()]; !ok {
		err := OrderStatusValidationError{
			field:  "Status",
			reason: "value must be in list [WAIT_BUYER_PAY PAYING TRADE_SUCCESS TRADE_CLOSED TRADE_FINISHED]",
		}
		if !all {
			return err
		}
...
var _OrderStatus_Status_InLookup = map[string]struct{}{
	"WAIT_BUYER_PAY": {},
	"PAYING":         {},
	"TRADE_SUCCESS":  {},
	"TRADE_CLOSED":   {},
...
func (m *CartItemRequest) validate(all bool) error {
	...
	if m.GetUserId() <= 0 {
		err := CartItemRequestValidationError{
			field:  "UserId",
			reason: "value must be greater than 0",
		}
		if !all {
	...
	if val := m.GetNums(); val <= 0 || val > 999 {
		err := CartItemRequestValidationError{
			field:  "Nums",
...
func (m *OrderRequest) validate(all bool) error {
	...
	if !_OrderRequest_Mobile_Pattern.MatchString(m.GetMobile()) {
		err := OrderRequestValidationError{
			field:  "Mobile",
	...
	if utf8.RuneCountInString(m.GetPost()) > 20 {
		err := OrderRequestValidationError{
			field:  "Post",
...
var _OrderRequest_Mobile_Pattern = regexp.MustCompile("^1[3-9]\\d{9}$")
...
func (m *OrderFilterRequest) validate(all bool) error {
	...
	if m.GetPages() < 1 {
		err := OrderFilterRequestValidationError{
			field:  "Pages",
	...
	if val := m.GetPagePerNums(); val < 1 || val > 100 {
		err := OrderFilterRequestValidationError{
			field:  "PagePerNums",
